...
mkdir -p /config/couchpotato /config/sabnzbd /config/sickbeard /downloads/movies /downloads/tv-shows
SAPAPI_KEY=$(< /dev/urandom tr -dc a-z0-9  | head -c${1:-32})
fi
...
sed -i  "/create_group_folders = 0/{n;s/.*/api_key =${SAPAPI_KEY}/}" /config/sabnzbd/sabnzbd.ini
fi
...
sed -i -e "s@sab_apikey.*@sab_apikey = \"${SAPAPI_KEY}\"@g" /config/sickbeard/config.ini
fi
